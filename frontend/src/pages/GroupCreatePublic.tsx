...
export default function GroupCreatePublic() {
    ...
    const [participants, setParticipants] = useState<Participant[]>([{ username: '', email: '' }]);

    // Navbar : on n'affiche pas le nom / logout pour les publics
    const handleDummyLogout = () => navigate('/login');
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                const { data } = await api.get(`/public/projects/${slug}`); // ⬅️ endpoint public
                if (!mounted) return;
    ...
    function addRow() { setParticipants((p) => [...p, { username: '', email: '' }]); }
    function updateRow(i: number, next: Participant) { setParticipants((p) => p.map((x, idx) => idx === i ? next : x)); }
    function removeRow(i: number) { setParticipants((p) => p.filter((_, idx) => idx !== i)); }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            setSubmitting(true);
            const { data } = await api.post(`/public/projects/${slug}/groups`, { participants });
            alert(`Groupe #${data.group.number} créé avec succès !`);
            navigate(`/invite/${slug}`); // ou rester sur place
        } catch (e: any) {
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <header className="mb-3">
                            ...
                            <p className="text-muted mb-0">{project.description || 'aucune description'}</p>
                        ...
                        <form onSubmit={handleSubmit} className="card shadow-sm">
                            <div className="card-body">
                                ...
                                <button type="button" className="btn btn-outline-secondary mt-2" onClick={addRow}>
